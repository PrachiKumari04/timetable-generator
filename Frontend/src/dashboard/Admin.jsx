...
export default function Admin() {
  const [activeTab, setActiveTab] = useState("dashboard"); // dashboard, master, schedule
  // which sub-entity is active in Master Data
  const [activeEntity, setActiveEntity] = useState(MASTER_ENTITY_KEYS[0]);

  // Actually load from server!
  const [masterData, setMasterData] = useState(createEmptyMasterData());

  const fetchMasterData = async () => {
    try {
      const endpoints = {
        program: fetch("http://localhost:5000/api/v1/programs").then(r => r.json()),
        room: fetch("http://localhost:5000/api/v1/rooms").then(r => r.json()),
        section: fetch("http://localhost:5000/api/v1/sections").then(r => r.json()),
        Specialization: fetch("http://localhost:5000/api/v1/specializations").then(r => r.json()),
        faculty: fetch("http://localhost:5000/api/v1/faculty").then(r => r.json()),
        student: fetch("http://localhost:5000/api/v1/students").then(r => r.json()),
      };
      ...
      console.error("Failed to load master data entirely.", e);
    }
  };
  ...
  const handleEntitySubmit = (e) => {
    ...
    const apiPath = `http://localhost:5000/api/v1/${activeEntity === "faculty" ? "faculty" : activeEntity + "s"}`;

    if (editingEntityId) {
      // Optional PUT functionality for editing if you add it later
      alert("Editing functionality via API is not currently supported in this version.");
    } else {
      // POST request to create new entity
      fetch(apiPath, {
        method: "POST",
        ...
        .catch(err => {
          console.error(err);
          alert("Error saving entity. Is the backend running?");
        });
    }
  ...
  const handleTtSave = () => {
    ...
    // Optional: Convert local React TTGrid to matching database schema if using custom format
    // For now, post the entire grid payload
    fetch("http://localhost:5000/api/v1/timetable/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        semester_id: "auto-generated", // placeholder since frontend doesn't supply this yet
        class_id: selectedClassForTT,
        grid: ttGrid
      })
    })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          ...
          alert("Saved locally (Server algorithm dummy success)");
          setTimetables((prev) => ({
            ...prev,
            [selectedClassForTT]: {
      ...
      .catch(err => {
        console.error(err);
        alert("Timetable saved locally due to API connection err.");
        setTimetables((prev) => ({
          ...prev,
          [selectedClassForTT]: {
            classCode: selectedClassForTT,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg border ${activeTab === "master"
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg border ${activeTab === "timetable"
              ? "bg-indigo-600 text-white border-indigo-600"
              : "bg-white text-gray-700 border-gray-300"
              }`}
          >
              ...
              {MASTER_ENTITY_KEYS.map((key) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg border ${activeEntity === key
